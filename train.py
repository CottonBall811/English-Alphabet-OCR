...
class CrossEntropyLoss:
    ...
    def forward(self,x,label):
        ...
        for a,b in zip(self.label, label):
            a[b] = 1.0
        self.loss = np.sum(np.nan_to_num(-self.label * np.log(x) - (1 - self.label) * np.log (1 - x))) / self.x.shape[0]
        return self.loss
...
def main():
    ...
    inner_layers.append(FullyConnect(26, 26))
    inner_layers.append(Sigmoid())
    ...
    for i in range(epochs):
        ...
        while True:
            ...
            if pos == 0:
                ...
                print('accuracy:', accu)
                break
    np.savetxt("OCR_weights.csv",inner_layers[0].weights,delimiter=',')
    np.savetxt("OCR_bias.csv",inner_layers[0].bias,delimiter=',')
